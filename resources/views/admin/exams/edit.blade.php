...
<div class="bg-white rounded-lg shadow p-6">
    ...
    <form method="POST" action="{{ route('admin.exams.update', $exam) }}">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="4" class="w-full px-3 py-2 border rounded">{{ old('description', $exam->description) }}</textarea>
        </div>

        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                <label class="block text-gray-700 font-bold mb-2">Duration (minutes)</label>
                <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $exam->duration_minutes) }}" required
            ...
            <div>
                ...
                <input type="number" step="0.01" name="passing_percentage" value="{{ old('passing_percentage', $exam->passing_percentage) }}" required
        ...
        <div class="mb-4">
            ...
            <select name="status" class="w-full px-3 py-2 border rounded" required>
                <option value="draft" {{ old('status', $exam->status->value) === 'draft' ? 'selected' : '' }}>Draft</option>
                <option value="published" {{ old('status', $exam->status->value) === 'published' ? 'selected' : '' }}>Published</option>
                <option value="archived" {{ old('status', $exam->status->value) === 'archived' ? 'selected' : '' }}>Archived</option>
            </select>
        </div>
